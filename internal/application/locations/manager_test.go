...
func TestDeleteLocation_WithValidKey_ShouldRemoveSectionFromGitConfigAndDeleteConfigFile(t *testing.T) {
	locationMgr := newMockLocationManager(gitConfigContent)
	key := "location1"

	// Verify it exists before deletion and pre-create its config file in the in-memory FS
	l, err := locationMgr.FindLocationByKey(key)
	require.Nil(t, err)
	require.NotNil(t, l)
	require.Nil(t, aFS.WriteFile(l.ConfigFile, []byte(""), fs.ModeAppend))

	err = locationMgr.DeleteLocation(key)
	require.Nil(t, err)

	// Section must no longer be present in the gitconfig file
	remaining, err := locationMgr.FindLocationByKey(key)
	require.Nil(t, err)
	assert.Nil(t, remaining)

	// Total location count must decrease by one
	locations, err := locationMgr.GetLocations()
	require.Nil(t, err)
	assert.Equal(t, 2, len(locations))

	// The associated config file must have been removed
	_, statErr := aFS.Stat(l.ConfigFile)
	assert.True(t, errors.Is(statErr, os.ErrNotExist))
}

func TestDeleteLocation_WithNonExistingKey_ShouldReturnError(t *testing.T) {
	locationMgr := newMockLocationManager(gitConfigContent)
	err := locationMgr.DeleteLocation("nonexistent")

	assert.NotNil(t, err)
}

func TestDeleteLocation_WithInvalidFile_ShouldReturnError(t *testing.T) {
	AFS := filesystem.NewMemFs()
	locationMgr := NewLocationManager("/non/existent/path", AFS)
	err := locationMgr.DeleteLocation("location1")
